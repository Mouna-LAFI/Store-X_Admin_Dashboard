...
if (isset($_GET['page_no']) && $_GET['page_no']!="") {
	...
	$result_count = mysqli_query($con,"SELECT COUNT(*) As total_records FROM `contact`");
	$total_records = mysqli_fetch_array($result_count);
	$total_records = $total_records['total_records'];
    $total_no_of_pages = ceil($total_records / $total_records_per_page);
	$second_last = $total_no_of_pages - 1; // total page minus 1

    $result = mysqli_query($con,"SELECT * FROM `contact` LIMIT $offset, $total_records_per_page");
...
while($donnees = mysqli_fetch_array($result))  
{
 ?>
   ...
   <td><?php echo $donnees['message'];?></td>
	...
	<td>
<a href="supprimer_messages.php?id=<?php echo $donnees['id'];?>" onclick="return confirm('Voulez-vous supprimer ce message ?');"> 
